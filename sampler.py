...
class ResShiftSampler(BaseSampler):
    ...
    def inference_tensor(
        ...
        """
        Super-resolve a single in-memory tensor and save a PNG.
        Args:
            image_tensor: torch.Tensor, shape [1, C, H, W], values in [0, 1]
            save_dir: pathlib.Path or str
            file_name: str, e.g. "slice0001_SR.png"
            mask_tensor: optional torch.Tensor [1,1,H,W] in [0,1] for inpainting-style blending
            mask_back: if True, blends SR with LQ where mask==0 (like your inference())
            noise_repeat: bool, pass-through to sample_func
        Returns:
            im_sr_tensor: torch.Tensor [1, C, H*sf, W*sf] in [0,1]
        """
        save_dir = Path(save_dir)
        save_dir.mkdir(parents=True, exist_ok=True)

        assert isinstance(image_tensor, torch.Tensor) and image_tensor.ndim == 4, \
            "image_tensor must be [1,C,H,W] torch.Tensor"
        im_lq_tensor = image_tensor.cuda().float()
        # scale to [-1,1] as in your inference() path
        ...
        # Tiled inference path (mirrors your _process_per_image logic)
        if im_lq_tensor.shape[2] > self.chop_size or im_lq_tensor.shape[3] > self.chop_size:
            if mask_tensor is not None:
                im_lq_cat = torch.cat([im_lq_tensor, mask_tensor], dim=1)
            else:
                im_lq_cat = im_lq_tensor
        ...
        if mask_back and mask_tensor is not None:
            mask_vis = mask_tensor * 0.5 + 0.5
            lq_vis = im_lq_tensor * 0.5 + 0.5
            im_sr_tensor = im_sr_tensor * mask_vis + lq_vis * (1 - mask_vis)

        # Save PNG (handles 1ch or 3ch)
        out = im_sr_tensor.detach().cpu().clamp(0, 1)
        if out.ndim == 4:
            out = out[0]  # [C,H,W]
        if out.shape[0] == 1:
            img = (out[0].numpy() * 255.0).round().astype(np.uint8)  # HxW
            cv2.imwrite(str(Path(save_dir) / file_name), img)
        else:
            img = (out.permute(1, 2, 0).numpy() * 255.0).round().astype(np.uint8)  # HxWxC (RGB)
            img_bgr = img[..., ::-1]  # cv2 expects BGR
            cv2.imwrite(str(Path(save_dir) / file_name), img_bgr)
