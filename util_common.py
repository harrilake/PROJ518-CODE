...
def get_filenames(dir_path, exts=['.nii', '.nii.gz', '.dcm'], recursive=True):
    '''
...
def scan_files_from_folder(dir_paths, exts, recursive=True):
    '''
    Scan files from given folder(s) with given extension(s).
    Case-insensitive. Accepts leading dots in extensions.
    '''
    ...
    for current_dir in dir_paths:
        for current_ext in exts:
            current_ext = current_ext.lower().lstrip('.')  # ✅ Normalize extension
            if recursive:
