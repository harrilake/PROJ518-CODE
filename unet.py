...
class Upsample(nn.Module):
    ...
    def __init__(self, channels, use_conv, dims=2, out_channels=None):
        ...
        self.dims = dims
        if use_conv:
            self.conv = conv_nd(2, self.channels, self.out_channels, 3, padding=1)
    ...
    def __init__(
        ...
        for level, mult in enumerate(channel_mult):
            for _ in range(num_res_blocks[level]):
                ...
                self.input_blocks.append(TimestepEmbedSequential(conv_nd(dims, 2, ch, 3, padding=1)))
...
class UNetModelSwin(nn.Module):
    ...
    def __init__(
        self,
        image_size,
        model_channels,  # Non-default argument must come before default ones
        num_res_blocks,  # Non-default argument must come before default ones
        attention_resolutions,  # Non-default argument must come before default ones
        in_channels=1,  # Default arguments should come after all non-defaults
        out_channels=1,  
        dropout=0,
        channel_mult=(1, 2, 4, 8),
        conv_resample=True,
