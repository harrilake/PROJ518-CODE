...
def get_named_eta_schedule(
    ...
    if schedule_name == 'exponential':
        ...
        sqrt_etas = np.linspace(min_noise_level, etas_end, num_diffusion_timesteps, dtype=np.float64)
...
def _extract_into_tensor(arr, timesteps, broadcast_shape):
    ...
    """
    res = th.from_numpy(arr).to(device=timesteps.device, dtype=timesteps.dtype)[timesteps]
...
class GaussianDiffusion:
    ...
    def encode_first_stage(self, y, first_stage_model, up_sample=False):
        ...
        if not torch.is_grad_enabled():  # ✅ Changed line
            with th.no_grad():
    ...
    def training_losses(
        ...
        #print("z_start.requires_grad?", z_start.requires_grad)
        #print("z_y.requires_grad?", z_y.requires_grad)

        if noise is None:
            noise = th.randn_like(z_start)
       
        if z_y.shape[-2:] != z_start.shape[-2:]:
            z_y = F.interpolate(z_y, size=z_start.shape[-2:], mode='bilinear', align_corners=False)

        #print("x_start.requires_grad?", x_start.requires_grad)
        ...
        if self.loss_type == LossType.MSE or self.loss_type == LossType.WEIGHTED_MSE:
            ...
            #print("model_output.requires_grad?", model_output.requires_grad)
            target = {
        ...
        #print("pred_zstart.requires_grad?", pred_zstart.requires_grad)
