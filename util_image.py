...
def batch_PSNR(img, imclean, border=0, ycbcr=False):
    ...
    # Convert to uint8 images in [0, 255]
...
def imread(path, chn='rgb', dtype='float32', force_gray2rgb=True, force_rgba2rgb=False):
    '''
    Read image. Supports NIfTI MRI files (.nii/.nii.gz) and standard images.
    '''
    path_str = str(path)
    
    if path_str.endswith('.nii') or path_str.endswith('.nii.gz'):
        # Load NIfTI MRI file
        nii_img = nib.load(path_str)
        im = nii_img.get_fdata()

        # Normalize to [0, 1], then scale to [0, 255] uint8
        im = (im - np.min(im)) / (np.max(im) - np.min(im) + 1e-8)
        ...
        try:
            im = cv2.imread(path_str, cv2.IMREAD_UNCHANGED)  # BGR, uint8
        except:
            print(f"Failed to read {path_str}")
            im = None

        if im is None:
            raise FileNotFoundError(f"Could not load image: {path_str}")

        if chn.lower() == 'gray':
            assert im.ndim == 2, f"{path_str} has {im.ndim} channels!"
        else:
            if im.ndim == 2 and force_gray2rgb:
                im = np.stack([im, im, im], axis=2)
            elif im.ndim == 4 and force_rgba2rgb:
                im = im[:, :, :3]
            if chn.lower() == 'rgb':
                im = bgr2rgb(im)
