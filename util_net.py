...
def reload_model(model, ckpt):
    # Handle full checkpoint dicts with 'state_dict' key
    if 'state_dict' in ckpt:
        ckpt = ckpt['state_dict']
    else:
        print("[⚠️ Warning] 'state_dict' not found — assuming raw checkpoint format.")
    ...
    for source_key, source_value in model.state_dict().items():
        ...
        if target_key not in ckpt:
            print(f"[❌ Missing] {target_key} not found in checkpoint — skipping")
            continue
